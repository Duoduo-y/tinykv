...
const (
	...
	LOG_LEVEL_FATAL = LOG_LEVEL_NONE | LogLevel(LOG_FATAL)    //1
	LOG_LEVEL_ERROR = LOG_LEVEL_FATAL | LogLevel(LOG_ERROR)   //11
	LOG_LEVEL_WARN  = LOG_LEVEL_ERROR | LogLevel(LOG_WARNING) //111
	LOG_LEVEL_INFO  = LOG_LEVEL_WARN | LogLevel(LOG_INFO)     //1111
	LOG_LEVEL_DEBUG = LOG_LEVEL_INFO | LogLevel(LOG_DEBUG)    //1 1111
...
func (l *Logger) diy(t LogType, tag string, v ...interface{}) {
	l.diyf(t, tag, "%v\n", v)
}
...
func DIY(t LogType, tag string, v ...interface{}) {
	_log.diy(t, tag, v...)
}
